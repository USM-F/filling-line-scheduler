...
`decomposition.py` partitions the bipartite eligibility graph into connected components. They share no products or lines, so demand, routes and calendar constraints remain independent. Components run sequentially within one budget, carrying feasible incumbents and native lower bounds across attempts.

For $F_1,F_2,F_4$, global values and lower bounds are sums; all component optima must be proven. For $F_3$, both aggregate by maximum. A matching global bound proves the global makespan without requiring every local minimum. Once $M^*$ is proven, impose $M_c\le M^*$ on every component and minimize local start sums under that common cap. This preserves the global lexicographic objective. Partition integrity and exact demand are checked on merge; the full schedule is independently validated.
