...
def test_supplied_baseline(input_path, capsys, decomposition):
    ...
    assert metrics["solver_elapsed_ms"] < 330000
...
def test_baseline_comparison():
    """Run after both baselines; compare proofs rather than demanding equal incumbents."""
    ...
    for left, right in zip(mono["passes"], decomposed["passes"]):
        if left["proven_optimal"] and right["proven_optimal"]:
