...
Цели по приоритету: переналадки → дополнительные назначения SKU на линии → makespan →
сумма стартов. Следующая цель решается после доказательства оптимума предыдущей.
...
[Полная математическая модель, проверка и baseline](docs/milp_model.md).
